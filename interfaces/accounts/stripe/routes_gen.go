...
func NewRoutes(p *props.ControllerProps, router *echo.Group, opts ...io.Writer) *Routes {
	...
	router.POST("", r.Post(p))
	return r
}

// Post ...
func (r *Routes) Post(p *props.ControllerProps) echo.HandlerFunc {
	i := NewPostController(p)
	...
	return func(c echo.Context) error {
		...
		if bindable {
			...
			if err := c.Bind(req); err != nil {
				log.Printf("failed to JSON binding(/accounts/stripe): %+v", err)
...
type IPostController interface {
	Post(c echo.Context, req *PostRequest) (res *PostResponse, err error)
}
