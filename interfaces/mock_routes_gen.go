...
func NewMockRoutes(router *echo.Group, jsonDir string, w io.Writer) *MockRoutes {
	...
	{
		jd := filepath.Join(jsonDir, "get_echo_message")
		router.GET(":message", r.GetEchoMessage(jd))
	}
	return r
}

// GetEchoMessage ...
func (r *MockRoutes) GetEchoMessage(jsonDir string) echo.HandlerFunc {
	type Mock struct {
		Meta struct {
			Status       int                    `json:"status"`
			MatchRequest *GetEchoMessageRequest `json:"match_request"`
		} `json:"meta"`
	...
	return func(c echo.Context) error {
		...
		if ago != "" {
			if err := json.Unmarshal([]byte(ago), option); err != nil {
				log.Printf("failed to JSON Unmarshal to `Api-Gen-Option` header(/{message}): %+v", err)
				return c.JSON(http.StatusBadRequest, map[string]interface{}{
					"code":    http.StatusBadRequest,
